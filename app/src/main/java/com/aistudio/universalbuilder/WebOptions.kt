...
class WebOptions(context: Context) {
    ...
    fun restore(data: JSONObject) { clear(); data.keys().forEach { key -> set(key,data.optBoolean(key,false)) } }
    ...
    fun json(): JSONObject = JSONObject().apply {
        for (key in listOf("camera", "microphone", "library", "media", "landscape", "fullscreen", "location", "notifications", "vibration", "network", "splash")) put(key, enabled(key))
    }
}
