...
if [ -n "$META_FILE" ] && [ -f "$META_FILE" ]; then
    APP_NAME="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["appName"])' "$META_FILE")"
    PACKAGE_NAME="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["packageName"])' "$META_FILE")"
fi
...
mkdir -p "$WRAPPER/app/src/main/res/values"
mkdir -p "$OUTPUT_DIR"
...
python3 - "$APP_NAME" "$WRAPPER/app/src/main/res/values/strings.xml" <<'PY'
import sys
...
cp "$APK" "$OUTPUT_DIR/generated-app.apk"
